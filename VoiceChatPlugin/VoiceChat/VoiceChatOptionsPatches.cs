...
/// <summary>
/// Settings panel injected into the Options menu.
///   Page 0 – audio devices + basic room rules (walls, distance, line-of-sight)
///   Page 1 – remaining room rules (ghost hearing, vent, comms, etc.)
/// </summary>
[HarmonyPatch]
public static class VoiceChatOptionsPatches
{
    ...
    private static int _page; // 0 = devices + basic rules, 1 = extended rules
    ...
    // ── Page 0: audio devices + basic room rules ──────────────────────────────
    ...
    private static void DrawRulesPage(ref float y)
    {
        ...
        { VoiceChatConfig.SetHostImpostorHearGhosts(!VoiceChatConfig.HostImpostorHearGhosts); BroadcastAndRefresh(); });
        y -= RowH;

        MakeFlagRow(VoiceChatLocalization.Tr("hearInVent"), y, s.HearInVent, isHost, () =>
        { VoiceChatConfig.SetHostHearInVent(!VoiceChatConfig.HostHearInVent); BroadcastAndRefresh(); });
        y -= RowH;

        MakeFlagRow(VoiceChatLocalization.Tr("ventPrivateChat"), y, s.VentPrivateChat, isHost, () =>
        { VoiceChatConfig.SetHostVentPrivateChat(!VoiceChatConfig.HostVentPrivateChat); BroadcastAndRefresh(); });
        y -= RowH;

        MakeFlagRow(VoiceChatLocalization.Tr("commsSabDisables"), y, s.CommsSabDisables, isHost, () =>
        { VoiceChatConfig.SetHostCommsSabDisables(!VoiceChatConfig.HostCommsSabDisables); BroadcastAndRefresh(); });
        y -= RowH;

        MakeFlagRow(VoiceChatLocalization.Tr("cameraCanHear"), y, s.CameraCanHear, isHost, () =>
        { VoiceChatConfig.SetHostCameraCanHear(!VoiceChatConfig.HostCameraCanHear); BroadcastAndRefresh(); });
        y -= RowH;

        MakeFlagRow(VoiceChatLocalization.Tr("impostorPrivateRadio"), y, s.ImpostorPrivateRadio, isHost, () =>
        { VoiceChatConfig.SetHostImpostorPrivateRadio(!VoiceChatConfig.HostImpostorPrivateRadio); BroadcastAndRefresh(); });
        y -= RowH;

        MakeFlagRow(VoiceChatLocalization.Tr("onlyGhostsCanTalk"), y, s.OnlyGhostsCanTalk, isHost, () =>
        { VoiceChatConfig.SetHostOnlyGhostsCanTalk(!VoiceChatConfig.HostOnlyGhostsCanTalk); BroadcastAndRefresh(); });
        y -= RowH;

        MakeFlagRow(VoiceChatLocalization.Tr("onlyMeetingOrLobby"), y, s.OnlyMeetingOrLobby, isHost, () =>
        { VoiceChatConfig.SetHostOnlyMeetingOrLobby(!VoiceChatConfig.HostOnlyMeetingOrLobby); BroadcastAndRefresh(); });
        y -= RowH;
    }
    ...
    {
        string mark = value
            ? $"<color=#55ff55>{VoiceChatLocalization.Tr("flagOn")}</color>"
            : $"<color=#ff5555>{VoiceChatLocalization.Tr("flagOff")}</color>";
        MakeValueDisplay(y, label, mark);
        if (!isHost) return;
        MakeSmallBtn(
            value ? VoiceChatLocalization.Tr("flagOn") : VoiceChatLocalization.Tr("flagOff"),
    ...
    private static void MakeDistanceRow(float y, float value, bool isHost)
    {
        ...
        MakeSmallBtn("−", new Vector3(-1.87f, y, -0.5f), () =>
        {
            VoiceChatConfig.SetHostMaxChatDistance(Mathf.Clamp(value - 0.5f, 1.5f, 20f));
            BroadcastAndRefresh();
        });
        MakeSmallBtn("+", new Vector3(1.87f, y, -0.5f), () =>
        {
